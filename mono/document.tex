...
Uma dificuldade encontrada no trabalho foi o desenvolvimento do cliente utilizando a linguagem \emph{Ruby}. Nos experimentos, 10 clientes eram executados ao mesmo tempo em cada máquina. Por fatores de desempenho, a máquina ficava lenta e os resultados dos experimentos eram inconsistentes. 

Outra dificuldade encontrada foi a dificuldade da utilização do \emph{cluster}. Inicialmente, os arquivos estavam na pasta do usuário (\emph{home}) e por questões de configuração do sistema de arquivos os experimentos ficavam lentos, resultando em valores errôneos. Ao passar os arquivos para a pasta \emph{opt} das máquinas, as execuções fluíram de maneira normal. 
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\textwidth]{img/delay-graph.png}
  \caption{Tempos de atraso}
  \label{fig:delay-graph}
\end{figure}

Na Figura~\ref{fig:cpu-graph}, as cargas de \emph{CPU} dos experimentos executados estão à mostra. O eixo $x$ mostra o experimento correspondente. O eixo $y$ indica a porcentagem dos valores para carga no modo usuário, \emph{kernel} e ocioso.

\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\textwidth]{img/cpu-graph.png}
  \caption{Cargas de \emph{CPU}}
  \label{fig:cpu-graph}
\end{figure}

A quantidade de \emph{bytes} enviados e recebidos durante os experimentos está ilustrada na Figura~\ref{fig:net-graph}. No eixo $x$, o experimento está indiciado. Os valores dessas métricas estão representados no eixo $y$.

\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\textwidth]{img/delay-graph.png}
  \caption{\emph{Bytes} de entrada e saída}
  \label{fig:net-graph}
\end{figure}

Na Figura~\ref{fig:tcp-graph}, os tempos de atraso e suas variâncias para o jogo sendo executado no modo \emph{TCP} não bloqueante com pacotes de tamanho 100 estão apresentados. O eixo $x$ mostra o número de clientes conectados e o eixo $y$ o tempo em milisegundos. A curva gerada é aproximada por linhas de \emph{Bezier}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\textwidth]{img/tcp-graph.png}
